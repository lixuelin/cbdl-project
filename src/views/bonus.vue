  ...
  <div class="bonus">
    ...
    <div class="bonus-body">
      ...
      <div class="bonus-body-next">
        <ul>
          <li>
            <div class="bonus-body-next-part">用户账号</div>
            <div class="bonus-body-next-flex">
              <div class="bonus-body-next-grid">收益金额</div>
              <div class="bonus-body-next-grid">更新时间</div>
              <div class="bonus-body-next-grid">我的奖金</div>
            </div>
          </li>
...
export default {
  ...
  methods: {
    ...
    async cashBonus(user_id) {
      let data = {
        user_id,
        cash_num: this.bonus.total.toFixed(2)
      };
      if (
        localStorage.getItem("user_id") === "" ||
        localStorage.getItem("user_id") === null
    ...
    async showNext(item) {
      ...
      try {
        ...
        this.next_invest = res.data.bonus_list;
      } catch (error) {
        if (res.msg) {
          return this.$Message.error(`奖金列表获取失败:${res.msg}`);
        }
        this.$Message.error(`奖金列表获取失败:${error}`);
      }
    },
    async showNextUser() {
      let data = {
    ...
    async showNextNextUser(item) {
      ...
      try {
        ...
        this.second_user_list = res.data;
      } catch (error) {
        if (res.msg) {
          return this.$Message.error(`请求失败:${res.msg}`);
        }
