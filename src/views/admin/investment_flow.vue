        ...
        <div class="admin-main-search">
            <div class="admin-main-search-box">
                <div class="admin-main-search-box-fields">
                   <span>用户账号：</span> <Input v-model="search.username" placeholder="请输入用户账号" clearable style="width: 200px" />
                </div>
                <div class="admin-main-search-box-fields">
                    <span>银行卡号：</span> <Input v-model="search.card" placeholder="请输入银行卡号" clearable style="width: 200px" />
                </div>
                <div class="admin-main-search-box-fields">
                    <span>户主姓名：</span> <Input v-model="search.household" placeholder="请输入户主名称" clearable style="width: 200px" />
                </div>
            </div>
            <div class="admin-main-search-box">
                <div class="admin-main-search-box-fields">
            ...
            <div class="admin-main-search-box">
                ...
                <div class="admin-main-search-box-fields">
                    <div class="admin-main-search-box-btns">
                        <Button type="primary" @click="getInvestList">搜索</Button>
      ...
      <Modal v-model="change_invest" width="460">
        ...
        <div class="admin-main-verify">
          <div class="admin-main-verify-line">
            <label>用户择投资金额: </label><span>{{invest_info.invest_num}}</span>
          </div>
          <div class="admin-main-verify-line">
            <label>实际到账金额: </label><input type="text" v-model="invest_info.verify_num" placeholder="请输入实际到账金额！">
          </div>
        </div>
        <div slot="footer">
          <Button type="primary" size="large" long :loading="modal_loading" @click="verifyInvestNum">确认已到账</Button>
        </div>
      </Modal>
    </div>
</template>
...
<script>
	import {queryInvestList, queryBanks, downloadEveryExecl, updateVerify} from "./../../utils/admin_api";
    ...
    export default {
        ...
        methods: {
          ...
          verifyInvestNum(){
            ...
            if (this.invest_info.verify_num <= 0 || this.invest_info.verify_num === "" || isNaN(this.invest_info.verify_num)) {
              this.$Message.warning("请输入正确的投资金额！")
              return
            }

            updateVerify(data).then(response => {
